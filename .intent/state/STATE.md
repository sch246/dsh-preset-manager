...
- The last sealed candidate remains historical evidence for the earlier three compatibility regions. It is not selected by draft.19 because the current intent, source identity and Host compatibility contribution have changed; setup and uninstall still regenerate shared catalogs from the current source tree.
...
- No candidate LOCK currently binds draft.19 to a committed source identity and deployed evidence. A new candidate may be created only after the current code, Host patch, migration and live checks are committed and recorded.
...
- The root-workspace preset start ordering and historical preset projection migration require deployed interaction and cache-count evidence before PM-001 and PM-003 are accepted.
