...
`npm run typecheck` 和 `bash scripts/build.sh` 只检查类型与可发布产物，不构成行为验收。最终验收必须在真实 Web UI 和真实 settings 持久化上按 STATE 操作，并通过刷新与服务重启观察显式默认、最近 Session fallback、managed order、set/unset 和失败提示。
