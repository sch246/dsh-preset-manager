...
`;
		...
		*/
		function projectSeatOptions(roster, currentId) {
			return roster.filter((entry) => !entry.hidden && !entry.broken).map((entry) => ({
				...
				description: entry.description,
				defaultAction: entry.isDefault ? "default" : entry.id === currentId ? "set-default" : void 0
			}));
		}
		//#endregion
		...
		*/
		function SeatChip({ useRoster, useStore, useSeat, load, select, setDefault, t }) {
			const rosterSnapshot = useRoster((snapshot) => snapshot);
			...
			const roster = (0, react.useMemo)(() => deriveRoster(rosterSnapshot.presets, state), [rosterSnapshot.presets, state]);
			const stagedOptions = (0, react.useMemo)(() => projectSeatOptions(roster, seat.current), [roster, seat.current]);
			const chosenId = stagedOptions.some((option) => option.id === seat.current) ? seat.current : stagedOptions[0]?.id;
			const options = (0, react.useMemo)(() => chosenId === seat.current ? stagedOptions : projectSeatOptions(roster, chosenId ?? ""), [
				roster,
				stagedOptions,
				chosenId,
				seat.current
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Menu, {
					...
					items: options.map((option) => ({
						...
						label: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							className: `${css.menuItem}${option.defaultAction === "set-default" ? ` ${css.menuItemDefaultCandidate}` : ""}`,
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								...
								children: [
									...
									option.defaultAction === "default" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: css.menuItemDefault,
										children: t("seat.default")
									}),
									option.defaultAction === "set-default" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: css.menuItemDefaultAction,
										children: t("seat.setDefault")
									})
								]
							}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
		...
		*/
		...
		async function writeDefaultPreset(remote, id) {
			try {
		...
		async function unsetDefaultPreset(remote) {
			try {
				await remote.settings.mutate(AGENT_PRESET_SETTINGS_NS, [{
					op: "unset",
					path: ["default"]
				}], void 0);
			} catch (error) {
		...
		*/
		var RosterController = class {
			...
			async setDefault(actions, id) {
				const { presets } = this.store.getSnapshot();
				if (!presets.some((preset) => preset.id === id)) return "action.failed";
				if (await writeDefaultPreset(this.remote, id) !== void 0) return "action.failed";
