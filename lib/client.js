		...
		*/
		function derivePresetGroups(list, sessionNodes, roster, order, query) {
			...
			for (const node of sessionNodes) {
				...
				const presetId = summary.agentPreset !== void 0 && roster.some((entry) => entry.id === summary.agentPreset) ? summary.agentPreset : void 0;
...
`;
		...
		*/
		...
		function PresetStar({ active, onSelect, t }) {
		...
		*/
		function PresetGroups({ query, rows, sessionActions, useSessions, useWorkspaces, useStore, actions, useRoster, load, open, startSessionByPreset, setDefault, hide, unhide, rename, t }) {
			...
			const sessionNodes = (0, react.useMemo)(() => rows.deriveSessions(list, archivedSessionIds), [
				rows,
				list,
				archivedSessionIds
			]);
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [status, groups.map((group) => {
							...
							const projectRow = rows.renderProjectRow({
								...
								muted: group.hidden,
								leading: group.presetId === void 0 ? void 0 : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PresetStar, {
		...
		*/
		function apply(ctx) {
			...
			ctx.inject([
				...
				const waitForListed = (sessionId, timeoutMs = 3e3) => {
					if (scope.sessions.list.getSnapshot().byId[sessionId] !== void 0) return Promise.resolve(true);
					return new Promise((resolve) => {
						...
						const stop = scope.sessions.list.subscribe(() => {
							if (scope.sessions.list.getSnapshot().byId[sessionId] !== void 0) finish(true);
						});
						const finish = (ok) => {
							if (settled) return;
							settled = true;
				...
				startSessionByPreset = async (id, workspaceId) => {
					...
					const currentWorkspaceId = sessions.current === void 0 ? void 0 : workspaces.items.find((workspace) => workspace.sessionIds.includes(sessions.current))?.workspaceId;
					const target = workspaceId ?? currentWorkspaceId ?? workspaces.recentWorkspaceId;
					if (target === void 0) return "start.noWorkspace";
					const workspace = workspaces.items.find((item) => item.workspaceId === target);
					if ((workspace === void 0 ? void 0 : sessions.ids.find((id) => {
						const summary = sessions.byId[id];
						return summary !== void 0 && summary.blank && summary.cwd === workspace.path && workspace.sessionIds.includes(summary.id) && !workspaces.archivedSessionIds.includes(summary.id);
					})) !== void 0) {
					...
					try {
						const response = await scopeApi.sessions.create({
							workspaceId: target,
							agentPreset: id
						});
