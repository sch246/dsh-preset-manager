...

{
  ...
  echo "generators=node --import tsx/esm scripts/gen-client-catalog.ts|node --import tsx/esm scripts/gen-cordis-api.ts"
...
node "$CHECKOUT/apps/cli/lib/bin.js" plugin --profile "$DSH_PROFILE" add "$REPO_DIR/packages/dsh-preset-manager"

echo
echo "setup: complete; no service restart was performed"
