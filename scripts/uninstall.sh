...
if command -v dsh >/dev/null 2>&1; then
  ...
  (cd "$REPO_DIR" && dsh plugin --profile "${DSH_PROFILE:-web}" remove dsh-preset-manager) \
    || echo "uninstall: dsh plugin remove failed — remove 'dsh-preset-manager' manually"
elif command -v node >/dev/null 2>&1 && [ -f "$CHECKOUT_CLI" ]; then
  echo "removing bundle through the checkout CLI from profile '${DSH_PROFILE:-web}'..."
  (cd "$REPO_DIR" && node "$CHECKOUT_CLI" plugin --profile "${DSH_PROFILE:-web}" remove dsh-preset-manager) \
    || echo "uninstall: checkout CLI remove failed — remove 'dsh-preset-manager' manually"
elif command -v pnpm >/dev/null 2>&1; then
  echo "removing bundle through pnpm from profile '${DSH_PROFILE:-web}'..."
  (cd "$REPO_DIR" && pnpm --dir "$CHECKOUT" dsh plugin --profile "${DSH_PROFILE:-web}" remove dsh-preset-manager) \
    || echo "uninstall: checkout CLI remove failed — remove 'dsh-preset-manager' manually"
else
  echo "neither dsh nor pnpm is available; remove 'dsh-preset-manager' manually"
fi
